...
import { API_URL, downloadPresentationPPT, downloadUnitPlanDocx } from '../lib/api';
...
function getTitle(doc, type) {
  switch (type) {
    ...
    case 'presentation': return doc.topik || 'Presentasi';
  ...
  switch (type) {
    ...
    case 'unitplan':
      return [
        doc.mata_pelajaran,
        doc.tingkat_kelas,
        doc.jumlah_pertemuan &&
        `${doc.jumlah_pertemuan} pertemuan`,
      ].filter(Boolean);
    default:
      return [];
  }
...
// ─── DocCard ─────────────────────────────────────────────────────────────────
function DocCard({ doc, type, onPress, onDelete, onDownload }) {
...
export default function MyDocsScreen({ navigation }) {
  const { token } = useAuth();
  console.log('MYDOCS TOKEN =', token);
      ...
      [
        { text: 'Batal', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (!path) return;
            try {
              console.log('DELETE URL:', `${API_URL}${path}`);
  ...
  const handleDownload = (doc) => {
    if (doc.__type === 'presentation') {
      return downloadPresentationPPT(doc.id);
    }

    if (doc.__type === 'unitplan') {
      return downloadUnitPlanDocx(doc.id);
    }

    Alert.alert(
      'Info',
      'Download belum tersedia untuk tipe dokumen ini.'
    );
  };
