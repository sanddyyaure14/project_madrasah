 ...
 * Edit profil guru — semua field kecuali password & email
 * Field: nama_lengkap, nip, mata_pelajaran, jenjang, kurikulum, no_hp
 * Backend: PUT /api/guru/profile
 */
...
export default function EditProfileScreen({ navigation }) {
  ...
  async function fetchProfile() {
    ...
    try {
      const res = await fetch(`${API_URL}/guru/profile`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json();
      console.log('[EditProfile] profile data:', JSON.stringify(json.data, null, 2));
      if (json.success && json.data) {
        const p = json.data;
        setNamaLengkap(p.nama_lengkap || '');
        setNip(p.nip || '');
        // mata_pelajaran bisa berupa array atau string dari DB
        const mapel = p.mata_pelajaran;
        if (Array.isArray(mapel)) setMapelSelected(mapel);
        else if (mapel && typeof mapel === 'string') setMapelSelected([mapel]);
        else setMapelSelected([]);

        setJenjang(p.jenjang || '');
        setKurikulum(p.kurikulum || '');
        setNoHp(p.no_hp || '');
      } else {
        setError('Gagal memuat profil: ' + (json.message || ''));
      }
    } catch (e) {
      console.error('[EditProfile] fetch error:', e.message);
  ...
  async function handleSave() {
    ...
    try {
      const payload = {
        ...
        nip: nip.trim() || null,
        mata_pelajaran: mapelSelected.length > 0 ? mapelSelected : null,
      ...
      if (json.success) {
        // Update nama di AuthContext juga
      ...
      <View style={[styles.card, S.shadow]}>
        ...
        <Field label="Mata Pelajaran" hint="Bisa pilih lebih dari satu">
          <ChipGroup
            ...
            selected={mapelSelected}
            onSelect={toggleMapel}
            multi
          />
          {mapelSelected.length > 0 && (
            <Text style={styles.selectedHint}>Terpilih: {mapelSelected.join(', ')}</Text>
          )}
        </Field>
        ...
        <Field label="Kurikulum">
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
            {KURIKULUM_OPTIONS.map(opt => {
              const active = kurikulum === opt.value;
              return (
                <TouchableOpacity
                  key={opt.value}
                  style={[styles.chip, active && styles.chipActive]}
                  onPress={() => setKurikulum(opt.value)}
                  activeOpacity={0.75}
                >
