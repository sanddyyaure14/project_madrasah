...
// 4. GET ALL - Ambil semua rubrik milik user (atau semua jika kepsek)
const getAllRubrics = async (userId, isKepsek = false) => {
    if (isKepsek) {
        const query = `
            SELECT 
                ar.id,
...
// 5. GET BY ID - Ambil detail rubrik berdasarkan ID (kepsek bisa akses tanpa filter user)
const getRubricById = async (rubricId, userId, isKepsek = false) => {
    if (isKepsek) {
        const query = `
            SELECT 
                ar.id,
...
// 6. UPDATE - Update rubrik berdasarkan ID (kepsek bisa update tanpa filter user)
const updateRubric = async (rubricId, userId, data, isKepsek = false) => {
    if (isKepsek) {
        const query = `
            UPDATE assessment_rubric
            SET 
                jenis_tugas = $1,
                aspek_penilaian = $2,
                skala_nilai = $3,
...
// 7. DELETE - Hapus rubrik berdasarkan ID (kepsek bisa hapus tanpa filter user)
const deleteRubric = async (rubricId, userId, isKepsek = false) => {
    if (isKepsek) {
        const rubric = await pool.query('SELECT id, request_id FROM assessment_rubric WHERE id = $1', [rubricId]);
