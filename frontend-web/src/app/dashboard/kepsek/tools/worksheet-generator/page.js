...
export default function KepsekWorksheetGeneratorPage() {
  ...
  const handleGenerate = async (e) => {
    ...
    setError(''); setLoading(true); setLks(null); setWorksheetId(null);
    try {
      ...
      const response = await fetch(`${apiUrl}/api/worksheet/generate-worksheet`, {
        ...
        body: JSON.stringify({ mata_pelajaran: mapel.trim(), topik: topik.trim(), tingkat_kelas: kelas, durasi_menit: durasi, tipe_aktivitas: tipeSelected, tujuan_pembelajaran: tujuan.trim() }),
      });
  ...
  const handleCetakPDF = async () => {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || "";
    try {
      const res = await fetch(`${apiUrl}/api/worksheet/worksheets/${worksheetId}/cetak-pdf`, { headers: { "Authorization": `Bearer ${token}` } });
      if (!res.ok) throw new Error("Gagal mengunduh PDF");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `LKS_${lks?.judul || 'worksheet'}.pdf`; a.click();
      URL.revokeObjectURL(url);
    } catch (err) { alert("Gagal unduh PDF: " + err.message); }
  };

  function handleReset() { setLks(null); setWorksheetId(null); setError(''); }
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col" style={{ height: 'calc(100vh - 200px)' }}>
          <div className="flex-1 overflow-y-auto">
            <form onSubmit={handleGenerate} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-2">Mata Pelajaran <span className="text-red-500">*</span></label>
                <input type="text" value={mapel} onChange={e => { setMapel(e.target.value); if (error) setError(''); }}
                  placeholder="cth. Akidah Akhlak" className="w-full text-sm p-3 border border-gray-200 rounded-xl outline-none focus:border-emerald-500 bg-gray-50" />
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {MAPEL_SUGGESTIONS.map(s => (
                    <button key={s} type="button" onClick={() => setMapel(s)}
                      className="text-[10px] px-2.5 py-1 rounded-full border border-gray-200 text-gray-500 hover:border-emerald-400 hover:text-emerald-700 transition">{s}</button>
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-2">Topik Bahasan <span className="text-red-500">*</span></label>
                <input type="text" value={topik} onChange={e => { setTopik(e.target.value); if (error) setError(''); }}
                  placeholder="cth. Akhlak Terpuji kepada Orang Tua" className="w-full text-sm p-3 border border-gray-200 rounded-xl outline-none focus:border-emerald-500 bg-gray-50" />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {KELAS_OPTIONS.map(opt => (
                    <button key={opt} type="button" onClick={() => setKelas(opt)}
                      className={`px-4 py-2 rounded-full text-sm border transition font-medium ${kelas === opt ? 'bg-[#006747] text-white border-[#006747]' : 'bg-white text-gray-700 border-gray-200 hover:border-emerald-400'}`}>{opt}</button>
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-2">Durasi (Menit)</label>
                <input type="number" value={durasi} onChange={e => setDurasi(parseInt(e.target.value) || 0)}
                  className="w-full text-sm p-3 border border-gray-200 rounded-xl outline-none focus:border-emerald-500 bg-gray-50" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-2">Tipe Aktivitas <span className="text-xs text-gray-400">(bisa lebih dari satu)</span></label>
                <div className="flex flex-wrap gap-2">
                  {TIPE_OPTIONS.map(opt => (
                    <button key={opt} type="button" onClick={() => toggleTipe(opt)}
                      className={`px-4 py-2 rounded-full text-sm border transition font-medium capitalize ${tipeSelected.includes(opt) ? 'bg-[#006747] text-white border-[#006747]' : 'bg-white text-gray-700 border-gray-200 hover:border-emerald-400'}`}>{opt}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <textarea rows={2} value={tujuan} onChange={e => setTujuan(e.target.value)}
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Menyusun LKS via Groq AI...</> : <>🛠️ Generate LKS</>}
              </button>
            </form>
          </div>
        </div>
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50 shrink-0">
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={handleCetakPDF}
                      className="flex items-center gap-1.5 text-xs font-semibold text-white bg-[#006747] hover:bg-emerald-800 px-3 py-1.5 rounded-lg transition">⬇️ Unduh PDF</button>
                    <button type="button" onClick={handleReset}
                      className="text-xs text-gray-400 hover:text-gray-600 px-2 py-1.5 transition">✕ Hapus</button>
                  </div>
                </div>
                        ...
                        <div key={s.no}>
                          ...
                          {Array.isArray(s.opsi) && s.opsi.length > 0 ? (
                            <ul className="mt-1.5 ml-4 space-y-1">
                              {s.opsi.map((opsi, j) => <li key={j} className="text-gray-600">{String.fromCharCode(65 + j)}. {opsi}</li>)}
                            </ul>
