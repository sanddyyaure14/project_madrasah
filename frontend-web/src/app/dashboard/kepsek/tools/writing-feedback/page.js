...
function buildShareText(data) {
  ...
  text += `Nama   : ${data.nama_siswa || 'Siswa Anonim'}\nKelas  : ${data.tingkat_kelas}\nJenis  : Teks ${data.jenis_tulisan}\nSkor   : ${parseFloat(data.skor_total).toFixed(0)} / 100\n=====================================\n\n`;
...
export default function KepsekWritingFeedbackPage() {
  ...
  const handleGenerate = async (e) => {
    ...
    setError(''); setLoading(true); setResult(null); setRequestId(null);
    try {
  ...
  function handleReset() { setResult(null); setRequestId(null); setError(''); setTulisan(''); setNamaSiswa(''); setFokusSelected([]); }
    ...
    <div className="max-w-6xl mx-auto space-y-5 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col" style={{ height: 'calc(100vh - 200px)' }}>
          <div className="flex-1 overflow-y-auto">
            <form onSubmit={handleGenerate} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-semibold text-gray-800 mb-2">Teks Tulisan Siswa <span className="text-red-500">*</span></label>
                <textarea rows={6} value={tulisan} onChange={e => { setTulisan(e.target.value); if (error) setError(''); }}
                ...
                <div className="flex flex-wrap gap-2">
                  {JENIS_OPTIONS.map(opt => (
                    <button key={opt} type="button" onClick={() => setJenis(opt)}
                      className={`px-4 py-2 rounded-full text-sm border transition font-medium capitalize ${jenis === opt ? 'bg-[#006747] text-white border-[#006747]' : 'bg-white text-gray-700 border-gray-200 hover:border-emerald-400'}`}>{opt}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {FOKUS_OPTIONS.map(opt => (
                    <button key={opt} type="button" onClick={() => toggleFokus(opt)}
                      className={`px-4 py-2 rounded-full text-sm border transition font-medium ${fokusSelected.includes(opt) ? 'bg-[#006747] text-white border-[#006747]' : 'bg-white text-gray-700 border-gray-200 hover:border-emerald-400'}`}>{opt}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  {BAHASA_OPTIONS.map(opt => (
                    <button key={opt} type="button" onClick={() => setBahasa(opt)}
                      className={`px-4 py-2 rounded-full text-sm border transition font-medium ${bahasa === opt ? 'bg-[#006747] text-white border-[#006747]' : 'bg-white text-gray-700 border-gray-200 hover:border-emerald-400'}`}>{opt}</button>
                  ))}
                </div>
              </div>
              {error && (
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />AI sedang menganalisis tulisan siswa...</> : <>✨ Generate Feedback</>}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
              ...
              <>
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50 shrink-0">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button"
                      onClick={() => { const encoded = encodeURIComponent(buildShareText(result)); window.open(`https://wa.me/?text=${encoded}`, '_blank'); }}
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-1">
                      ...
                      {result.jenis_tulisan && (
                        <span className="inline-block mt-2 text-xs font-bold text-emerald-700 bg-emerald-50 rounded-full px-3 py-1 capitalize">Teks {result.jenis_tulisan}</span>
                      )}
                    </div>
          ...
          {result && requestId && (
            <RatingFeedback requestId={requestId} featureLabel="writing feedback" endpoint="writing" />
          )}
        </div>
      </div>
    </div>
