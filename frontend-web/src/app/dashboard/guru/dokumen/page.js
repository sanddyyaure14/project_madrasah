...
function RubricDetail({ doc, onDelete, onEdit }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button onClick={onEdit} className="flex-1 flex items-center justify-center gap-1.5 border border-gray-200 text-gray-600 text-sm font-semibold py-2 rounded-xl hover:bg-gray-50 transition">✏️ Edit</button>
        <button onClick={onDelete} className="flex-1 flex items-center justify-center gap-1.5 border border-red-200 text-red-500 text-sm font-semibold py-2 rounded-xl hover:bg-red-50 transition">🗑️ Hapus</button>
      </div>
    </div>
  );
}
